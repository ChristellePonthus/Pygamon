...
class MapManager:
    ...
    def __init__(self, screen, player):
        ...
        self.register_map("world",
                          ...
                          npcs=[
                              NPC("paul", nb_points=4, dialog=["Bonne aventure", "Je m'appelle Paul", "A+ !"]),
                              NPC("robin", nb_points=2, dialog=["J'espère que tu vas bien", "Je m'appelle Robin"])
                          ])
    ...
    def check_npc_collisions(self, dialob_box):
        for sprite in self.get_group().sprites():
            if sprite.feet.colliderect(self.player.rect) and type(sprite) is NPC:
                dialob_box.execute(sprite.dialog)
